...
void print_sort_s(vector<string> sort_s) {
    string out_s;
    for (const auto& s : sort_s) {
        out_s+= s + ",";
    }
    cout << out_s.substr(0, out_s.size() - 1)<< endl;
...
int main()
{
    ...
    { // 注意 while 处理多个 case
        ...
        while(!getline(line_stream, s, ',').fail())
        {
            sort_s.push_back(s);
        }
